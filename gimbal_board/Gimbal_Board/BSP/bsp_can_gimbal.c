...
void Can_Msg_Init(void)
{
    ...
    for (size_t i = 0; i < sizeof(buffer_list) / sizeof(buffer_list[0]); i++)
    {
        ...
        if (buffer_list[i].stdId == BIG_YAW_DM6006_TransID || buffer_list[i].stdId == BIG_PITCH_DM4340_TransID)
        {
...
void Ctrl_DM_Motor(float _pos, float _vel, float _KP, float _KD, float _torq) // CAN1
{
