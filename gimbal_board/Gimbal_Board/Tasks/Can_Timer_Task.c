...
TimerHandle_t DM_BIG_YAW_Timer_Handle; // 周期定时器句柄
...
void DM_BIG_YAW_Timer_Callback(TimerHandle_t xTimer)
{
    ...
    if (HAL_CAN_AddTxMessage(&hcan1, &big_yaw_send_msg.tx_header, big_yaw_send_msg.data, &send_mail_box) != HAL_OK)
    {
        if (HAL_CAN_AddTxMessage(&hcan1, &big_yaw_send_msg.tx_header, big_yaw_send_msg.data, &send_mail_box) != HAL_OK)
        {
            HAL_CAN_AddTxMessage(&hcan1, &big_yaw_send_msg.tx_header, big_yaw_send_msg.data, &send_mail_box);
        }
    }
}
...
void Create_Can_Send_Timers(void)
{
    ...
    // 创建达妙大yaw can发送定时器
    DM_BIG_YAW_Timer_Handle = xTimerCreate((const char *)"DM_BIG_YAW_Timer",
                                     (TickType_t)TIME_STAMP_1MS,                    // 1ms
                                     (UBaseType_t)pdTRUE,                           // 周期执行
                                     (void *)0,                                  
                                     (TimerCallbackFunction_t)DM_BIG_YAW_Timer_Callback); // 回调函数

    // 开启定时器,仅且仅能开启一次，否则会出错，不开启则不会发数据
    if (DM_BIG_YAW_Timer_Handle != NULL)
    {
        xTimerStart(DM_BIG_YAW_Timer_Handle, 0); // 不等待
    }
